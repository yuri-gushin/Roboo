...
sub handler ($) {
    ...
    # Generate cookie
    $valid_cookie = generate_cookie($settings->{challenge_hash_input}, $settings->{secret});
...
sub init ($) {
    ...
    if (not %{$settings}) {
        ...
        use Config;
        $settings->{randbits} = $Config{randbits};
        no Config;
    }
}
...
sub get_timeseed () {
    ...
    return int(rand(2**$settings->{randbits}-1));
}
...
sub challenge_GET_SWF ($) {
    ...
    if ($request->uri =~ /^\/(?:POST|GET)-$valid_cookie.swf$/) {
        if ($request->uri =~ /^\/GET-/) {
...
EOF
        $response =~ s/Roboo_value_PLACEHOLDER/GET-$valid_cookie/g;
        $request->send_http_header('text/html');
    }
...
EOF
    ...
    $response =~ s/VARIABLES_PLACEHOLDER/$form_variables/;
    $response =~ s/Roboo_value_PLACEHOLDER/POST-$valid_cookie/g;
    $request->send_http_header('text/html');
